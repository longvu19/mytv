...
<template>
  <div class="user-nav">
    <template v-if="user">
      ...
      <RouterLink @click.prevent="signOut(auth)" :to="{ force: false, hash: '', query: $route.query }" class="user-nav__link">Đăng xuất
      </RouterLink>
    </template>
    <template v-else>
      <RouterLink @click.prevent="togglePopup(true)" :to="{ force: false, hash: '', query: $route.query }">
        <ProfileImage />
      </RouterLink>
    </template>
  </div>
