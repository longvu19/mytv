<script lang="ts" setup>
  ...
  const footerLinkClassObject = computed(() => {
    return { "search-result__footer-link": props.isMobile }
  })
  const stopRequestPromise : Promise<null> = new Promise((resolve) => {
    if(props.stopRequest) {
      resolve(null);
    }
  })
  const searchResult: MovieSearchResultResponse | null = props.stopRequest ? await stopRequestPromise : await getMovieSearchResult(props.keyword, 10, abortController);
...
<template>
  <Suspense>
    ...
    <template #fallback>
      <Loading class="search-box__loading" :class="loadingClassObject" size="30px" type="square" />
    </template>
  </Suspense>
</template>
