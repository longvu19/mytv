    ...
    <template v-for="(movie) in slides" :key="movie.id">
      <SplideSlide class="master-carousel__item">
        ...
        <div class="master-carousel__info">
          ...
          <p class="master-carousel__o-title">{{ movie.origin_name }}</p>
          <p class="master-carousel__desc">{{ movie.description }}</p>
          <RouterLink :to="{ name: 'phim', params: { slug: movie.slug } }" class="main-menu__link">
            <Button type="button" class="master-carousel__button" size="large" icon="play" :primary="true">Xem
              ngay</Button>
          </RouterLink>
        </div>
      </SplideSlide>
